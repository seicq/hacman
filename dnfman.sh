...
install_packages() {
  ...
  if [ $? -eq 0 ]; then
    for package in $packages; do
      if sudo dnf install "^$package$" >/dev/null; then
...
remove_packages() {
  ...
  if [ $? -eq 0 ]; then
    sudo dnf remove $packages && sudo dnf autoremove 
...
search_packages() {
  ...
  if [ $? -eq 0 ]; then
    if sudo dnf search "$query" >/dev/null ; then
      results=$(sudo dnf search "$query" | awk '!/Checking for conflicting packages/')
      if [ -z "$results" ]; then
...
while true; do
  CHOICE=$(whiptail --title "DNF Package Manager" --menu "Choose an option:" 15 60 6 "${OPTIONS[@]}" 3>&1 1>&2 2>&3)
  case $CHOICE in
    "1.")
      install_packages
      ;;
